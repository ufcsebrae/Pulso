...
from comunicacao.carregamento import carregar_dataframe_para_sql_sem_duplicados # <-- IMPORT CORRETO
...
from processamento.validacao import (
    ...
    carregar_mapa_correcoes, # <-- AGORA LÊ DO SQL
    preparar_dados_para_validacao,
)

logger = logging.getLogger(__name__)

# ... (a função tratar_falhas_de_enriquecimento continua a mesma) ...
def tratar_falhas_de_enriquecimento(
    ...
    if args.modo_interativo:
        ...
        logger.info("Processo de correção finalizado. O mapa de correções foi atualizado no SQL.")
    else:
        print("\nPara corrigir as falhas, execute com a flag: python main.py --modo-interativo")

def run_pipeline(args: argparse.Namespace) -> None:
    df_orcado_raw, df_cc_raw = obter_dados_brutos()
    
    # ... (os passos 1, 2, 3 e 4 continuam os mesmos) ...
    ...
    colunas_presentes = [col for col in colunas_finais if col in df_enriquecido.columns]
    df_para_salvar = df_enriquecido[colunas_presentes].copy()
    
    df_para_salvar.dropna(subset=['CODCCUSTO'], inplace=True)

    # CHAMADA DA FUNÇÃO FINAL E CORRETA
    ...
    logger.info("SUCESSO! A tabela '%s' foi salva em '%s'.", NOME_TABELA_FINAL, engine_financa.url.database)

# ... (a função main() e o if __name__ == "__main__": continuam os mesmos) ...
def main() -> None:
